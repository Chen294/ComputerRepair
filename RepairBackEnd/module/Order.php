...
class Order {
    ...
    public function __construct($db, $conf){
        $this->_db = $db;
        $this->_conf = $conf;
        $this->_col =   [   $this->_conf['order_table'].".order_id",
                            $this->_conf['user_table'].".realname",
                            $this->_conf['user_table'].".phone",
        ...
        $this->_join =  [
                            [   
                                "[>]".$this->_conf['user_table']=>$this->_conf['uid'],
                            ],
                            [
                                "[>]".$this->_conf['computer_table']=>"computer_id",
                            ],
                            [
                                "[>]".$this->_conf['staff_table']=>'staff_id',
                            ],
                            [
                                "[>]".$this->_conf['user_table']=>
                                    [
                                        $this->_conf['staff_table'].".".$this->_conf['uid']=>$this->_conf['uid']
                                    ],
                            ],
                            [
    ...
    public function getUserOrders($user_id){
        $response = $this->_db->search( $this->_conf['order_table'],
                                        ...
                                        $this->_col,
                                        [
                                           $this->_conf['order_table'].".".$this->_conf['uid'] => $user_id,
                                           "ORDER" => ["time" => "DESC"]
                                        ]);
        $this->adjustDate($response);
        return $response;
    ...
    public function getOrderDetail($order_num){
        $response_basic = $this->_db->select($this->_conf['order_table'], "order_id", $order_num);
        ...
        $response_user = $this->_db->select($this->_conf['user_table'], $this->_conf['uid'], $data["basic"]["ucid"]);
        ...
        if ($data["basic"]["staff_id"] > 0){
            ...
            $response_staff_extend = $this->_db->select($this->_conf['user_table'], $this->_conf['uid'], $response_staff[0]["ucid"]);
    ...
    /**
     * @param string $id ucid or staff_id
     * @return array result
     */
