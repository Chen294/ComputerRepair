...
Flight::register('wechat', 'WeChat', array(Flight::db(), Flight::get('sysconfig')));
...
Flight::route('/api/login', function(){
    ...
    $_SESSION['basic'] = Flight::user()->getIsReg($uid);
    if ($_SESSION['basic'] == false){
        $new = 1;
        ...
        $new = 0;
        $_SESSION['basic']['staff_id'] = Flight::user()->getStaffId($uid);
    }
...
Flight::route('/api/user/new', function(){
    ...
    if (isset($_SESSION['uid'])){
        ...
        $resp = Flight::user()->addUser($_SESSION['uid'], $data);
        Flight::login()->updateData($_SESSION['uid'], $data);
        $_SESSION['basic'] = Flight::user()->getIsReg($_SESSION['uid']);
        Flight::json($resp);
    } else {
...
Flight::route('/api/wechat/code',function(){
    ...
    if ($wx['code'] == 200) {
        ...
        if ($isbind["code"] == 200){
            $profile = Flight::login()->getProfile($isbind["data"]["ucid"]);
            
            $_SESSION['uid'] = $isbind["data"]["ucid"];
            $_SESSION['profile'] = $profile['data'];
            //Flight::user()->updateData($_SESSION['uid'], $profile['data']);
            $_SESSION['basic'] = Flight::user()->getIsReg($_SESSION['uid']);
            $_SESSION['basic']['staff_id'] = Flight::user()->getStaffId($_SESSION['uid']);
...
Flight::route('/api/my/device', function(){
    Flight::json(array('code'=>200,
                   'data'=>Flight::order()->getUserDevice($_SESSION['uid'])
    ));
    return false;
});
...
Flight::route('/api/my/device/add', function(){
    ...
    if (empty($brand) || empty($model) || empty($date)){
        ...
        Flight::json(Flight::order()->addUserDevice($_SESSION['uid'], $brand, $model, $date));
    }
    return false;
});
...
Flight::route('/api/my/order', function(){
    if ($_SESSION['basic']['staff_id'] > 0){
        ...
        Flight::json(array('code'=>200,
                           'data'=>Flight::order()->getUserOrders($_SESSION['uid'])
            ));
    }
    return false;
});
...
Flight::route('/api/my/order/active', function(){
    Flight::json(array('code'=>200, 'data'=>count(Flight::order()->getUserActiveOrders($_SESSION['uid']))));
    return false;
});
...
Flight::route('/api/my/order/new', function(){
    ...
    if (count(Flight::order()->getUserActiveOrders($_SESSION['uid'])) > 0){
        Flight::json(array('code'=>403));
    } else {
        if ($device == 0){
            $req = Flight::order()->addUserDevice($_SESSION['uid'], $brand, $model, $date);
            if ($req["code"] != 200) {
                Flight::json($req);
                return false;
            } else {
